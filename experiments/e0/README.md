...
| `e0g_ilp_bench.py`(A/B 版) | 直空间深水区:`-prec-div/-prec-sqrt=false` 交替 A/B(2026-09-09) | **负结果 1.00×,力差 0.0**(代码生成未变);至此微杠杆全灭(展开/紧凑/重叠/fast-div);**结论:k2 ≈ 16 ms 为 replica-inner 布局实用下限**(坐标逻辑流量 33 GB/步,L2 后 ~2.05 TB/s;环形区 27% = skin 结构性代价);40% peak 需架构重构,挂 backlog |
