...
K = {n: mod.get_function(n) for n in ("k2_prefold", "p1_load", "p2_lj", "p3_full")}
...
def bench(kname, iters=30, block=128):
    ...
    for _ in range(iters):
        ...
        if u0 <= 20 and gpu_util() <= 20 and dt < 0.2:
            clean.append(dt)
    return min(clean) if clean else float("nan"), len(clean)


print(f"N={N} maxnb={maxnb} avg={ncnt.mean():.0f} (占用守卫: 迭代级, 重试环)")
import time as _time
res, attempt = {}, 0
ORDER = ("p3_full", "k2_prefold", "p2_lj", "p1_load")
